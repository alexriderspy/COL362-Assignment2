with a as (select id from tag where name in ('Frank_Sinatra','William_Shakespeare','Elizabeth_II','Adolf_Hitler','George_W._Bush')), 
b as (select personid, tagid from person_hasinterest_tag where tagid in (select * from a)), 
c as (select personid from b group by personid having count(distinct tagid) >= 2), 
postmsg as (select id, creatorpersonid from post where creationdate :varies and creationdate<'2011-07-19'), 
commmsg as (select id, creatorpersonid from comment where length :varies and length>100), 
...
msg_count_like as (select distinct personid from msg_like group by personid having count(*) >= 10),
...
f as (select p1id, p2id from e where (select count(*) from ((select distinct tagid from b where p1id = personid) intersect (select distinct tagid from b where p2id = personid)) as tb) >= 2),
...
l as (select p1id, p2id from k12 group by p1id, p2id having count(distinct msgid) >= 10),
